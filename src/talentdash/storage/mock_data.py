"""In-memory sample data when PostgreSQL is unavailable (local dev without Docker)."""
...
def company_summaries(page: int = 1, page_size: int = 20) -> tuple[list[dict], int]:
    ...
    for company, recs in sorted(by_company.items()):
        lpa_vals = [r.totalCompensation / 100_000 for r in recs]
...
def get_analytics() -> dict:
    ...
    for r in rows:
        lpa = r.totalCompensation / 100_000
    ...
    def _hist_buckets() -> list[dict]:
        ...
        for label, vals in hist.items():
            if label not in order and vals:
                result.append({"label": label, "count": len(vals), "avg_lpa": round(sum(vals) / len(vals), 2)})
        return result
    ...
    return {
        ...
        "by_role": by_role_out,
        "company_leaderboard": leaderboard,
    }
